...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity", "Activity Tracker", "Deal stage", "Original source", "Referral / No-Referral", "Activity concentration"],
...
def _render_performance_activity_concentration(
    ...
    dfw["_cohort_age_m"] = ((pd.Timestamp.today().to_period("M") - dfw["_ac_create"].dt.to_period("M")).astype("int")).fillna(0)
    ...
    with k4:
        med_age = int(dfw["_cohort_age_m"].median()) if len(dfw) else 0
