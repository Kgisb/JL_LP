...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity", "Activity Tracker", "Deal stage", "Original source", "Referral / No-Referral", "Lead mix", "Activity concentration"],
...
def _render_performance_lead_mix(
    ...
    # Resolve date columns for activity-style windows used by the referral view
    ...
    pay_col_resolved = pay_col

    # Create safe working copy with parsed dates
    df = df_f.copy()
    # Base dates
    for col in [create_col, pay_col_resolved, first_cal, cal_res, cal_done]:
        if col and col in df.columns:
            df[col] = pd.to_datetime(df[col], errors="coerce", dayfirst=True, utc=False, infer_datetime_format=True)

    # UI Controls — mirror referral A/B panel but for multiple sources as windows
    st.caption("Compare multiple windows (A, B, C, D, …), each defined by **JetLearn Deal Source**.")
    available_sources = []
    if _source and _source in df.columns:
        available_sources = sorted(df[_source].dropna().astype(str).unique().tolist())

    # A small helper to render a window config
    def window_cfg(label_key: str, default_src: str | None):
        st.markdown(f"#### Window {label_key}")
        c1, c2, c3, c4 = st.columns([1,1,1,1])
        with c1:
            src = st.selectbox(f"Deal Source ({label_key})", ["All"] + available_sources, index=(["All"]+available_sources).index(default_src) if default_src in (["All"]+available_sources) else 0, key=f"lm_src_{label_key}")
        with c2:
            mode = st.selectbox(f"Conversion% denominator ({label_key})", ["Deals Created", "First Cal Sched", "Cal Resched", "Cal Booked"], index=0, key=f"lm_den_{label_key}")
        with c3:
            start = st.date_input(f"Start ({label_key})", value=date.today().replace(day=1), key=f"lm_start_{label_key}")
        with c4:
            end = st.date_input(f"End ({label_key})", value=date.today(), key=f"lm_end_{label_key}")
        return {"src": src, "mode": mode, "start": start, "end": end}

    # Allow up to 6 windows by default (A..F). Pre-seed common sources if present.
    seeds = (["Referral","Event","Organic","PM - Search","PM - Social","Lead Magnet"] if available_sources else [])
    labels = list("ABCDEF")
    windows = []
    for i, lab in enumerate(labels):
        default_src = seeds[i] if i < len(seeds) else "All"
        with st.expander(f"Configure Window {lab}", expanded=(lab in ["A","B"])):
            windows.append(window_cfg(lab, default_src))

    # Build per-window filtered datasets
    def denom_series(df_in: pd.DataFrame, mode: str):
        if mode == "Deals Created":
            ser = df_in[create_col]
        elif mode == "First Cal Sched" and first_cal:
            ser = df_in[first_cal]
        elif mode == "Cal Resched" and cal_res:
            ser = df_in[cal_res]
        elif mode == "Cal Booked" and cal_done:
            ser = df_in[cal_done]
        else:
            ser = df_in[create_col]
    ...
    summaries = []
    for lab, cfg in zip(labels, windows):
        # Filter by source
        _dfw = df.copy()
        if _source and _source in _dfw.columns and cfg["src"] != "All":
            _dfw = _dfw[_dfw[_source].astype(str) == cfg["src"]]
        # Window on denominator dates
        ser = denom_series(_dfw, cfg["mode"])
        mask = (ser.dt.date >= cfg["start"]) & (ser.dt.date <= cfg["end"])
        _dfw = _dfw.loc[mask].copy()
        _dfw["__win"] = lab
        _dfw["__count"] = 1
        summaries.append(_dfw)

    if not summaries:
        st.info("No windows configured."); return
    df_all = pd.concat(summaries, ignore_index=True) if summaries else pd.DataFrame()

    total_rows = len(df_all)
    if total_rows == 0:
        st.info("No rows matched the configured windows."); return

    # KPI bar like referral view
    k1, k2, k3 = st.columns(3)
    with k1: st.metric("Total rows in windows", f"{total_rows:,}")
    with k2: st.metric("Windows compared", f"{df_all['__win'].nunique():,}")
    with k3:
        top_win = df_all['__win'].value_counts().head(1)
        top_lab, top_val = (top_win.index[0], int(top_win.iloc[0])) if not top_win.empty else ("—", 0)
        st.metric("Top window", top_lab, f"{top_val:,}")

    # Mix comparison (per-window composition) — Country, Counsellor, Source
    st.markdown("### Composition by window")
    for label, colname in [("Country","Country"), ("Counsellor", _counsellor if _counsellor else ""), ("Source", _source if _source else "")]:
        if not colname or colname not in df_all.columns:
            continue
        colname_real = colname
        tbl = (
            df_all.assign(**{colname_real: df_all[colname_real].fillna("Unknown").astype(str)})
                 .groupby(["__win", colname_real]).size().reset_index(name="count")
        )
        piv = tbl.pivot_table(index="__win", columns=colname_real, values="count", aggfunc="sum", fill_value=0)
        st.dataframe(piv, use_container_width=True)
        st.download_button(
            f"Download CSV — {label} by window",
            piv.reset_index().to_csv(index=False).encode("utf-8"),
            file_name=f"leadmix_{label.lower()}_by_window.csv",
            mime="text/csv",
            key=f"lm_dl_{label.lower()}"
        )

    # Trend / share chart — rows per window
    st.markdown("### Rows per window")
    win_counts = df_all['__win'].value_counts().reset_index()
    win_counts.columns = ["window","count"]
    ch = alt.Chart(win_counts).mark_bar().encode(x="window:N", y="count:Q", tooltip=["window","count"])
    st.altair_chart(ch, use_container_width=True)

    # Detailed download of concatenated selection
    st.download_button(
        "Download CSV — Lead mix selection",
        df_all.drop(columns=["__count"], errors="ignore").to_csv(index=False).encode("utf-8"),
        file_name="lead_mix_selection.csv",
        mime="text/csv",
        key="lm_dl_all"
    )
