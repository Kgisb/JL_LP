...
def _render_performance_lead_mix(
    ...
    all_sources = sorted(df[_source].dropna().astype(str).unique().tolist()) if _source and _source in df.columns else []
    def seed_or_all(name: str):
        return name if name in all_sources else "All"
    windows = [
        ("A", "All"),
        ("B", seed_or_all("Referral")),
        ("C", seed_or_all("Event")),
        ("D", seed_or_all("Organic")),
        ("E", seed_or_all("PM - Search")),
        ("F", seed_or_all("PM - Social")),
    ]

    # --- Legend for windows ---
    try:
        import pandas as _pd
        legend_df = _pd.DataFrame(windows, columns=["Window","JetLearn Deal Source"]).copy()
        legend_df["JetLearn Deal Source"] = legend_df["JetLearn Deal Source"].fillna("All").astype(str)
    ...
    for label, src in windows:
        dfw = df.copy()
        if _source and _source in dfw.columns and src != "All":
            dfw = dfw[dfw[_source].astype(str) == src]

        # Metrics
        leads = _mask(dfw, create_col).sum()
        enrolls = _mask(dfw, pay_col).sum()
        t_sched = _mask(dfw, first_cal).sum() if first_cal else 0
        t_resch = _mask(dfw, cal_res).sum() if cal_res else 0
        cal_done_ct = _mask(dfw, cal_done).sum() if cal_done else 0
        conv = round((enrolls / leads * 100.0), 1) if leads > 0 else 0.0

        rows.append({
            "Parameter": "Total leads", label: int(leads)
        })
        rows.append({
            "Parameter": "Total enrollments", label: int(enrolls)
        })
        rows.append({
            "Parameter": "Trial scheduled", label: int(t_sched)
        })
        rows.append({
            "Parameter": "Trial rescheduled", label: int(t_resch)
        })
        rows.append({
            "Parameter": "Calibration done", label: int(cal_done_ct)
        })
        rows.append({
            "Parameter": "Lead → Enrollment %", label: conv
        })
    ...
    if rows:
        # collapse by Parameter and pivot to have columns A..F
        param_df = pd.DataFrame(rows)
        param_df = param_df.groupby("Parameter").sum(numeric_only=True).reset_index()

        # UI layout: Left a fixed list of parameters for clarity, Right the table
...
- Lead → Enrollment %""")
    ...
    if not df_all.empty:
        ...
        # Apply the SAME windowing mask as for leads (Create Date) for the chart baseline
        m_leads = _mask(df_all, create_col)
        ...
        # Aggregate: counts by window x breakdown
        agg = (
            plot_df
            .groupby(["__win", bcol]).size().reset_index(name="count")
        )
        base = alt.Chart(agg).encode(
            x=alt.X("__win:N", title="Window (A..F)"),
            y=alt.Y("count:Q", title="Count (Create Date in range)"),
            color=alt.Color(f"{bcol}:N", title=breakdown),
            tooltip=["__win:N", alt.Tooltip(f"{bcol}:N", title=breakdown), "count:Q"]
        )
