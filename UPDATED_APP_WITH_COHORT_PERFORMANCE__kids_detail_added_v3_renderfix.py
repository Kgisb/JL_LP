...
# === MARKETING: Kids detail pill (S kid pattern) ===
...
def _guess_dealname_col(df):
    return _col_exists(df, ["Deal Name","Deal name","Name","Deal","Title"])

def _guess_source_col(df):
    return _col_exists(df, ["JetLearn Deal Source","Deal Source","Original source","Source","Original traffic source"])

def _guess_create_col(df):
    return _col_exists(df, ["Create Date","Created Date","Deal Create Date","Date Created","Created On","Creation Date","Deal Created Date","Create_Date"])

def _guess_enroll_col(df):
    return _col_exists(df, ["Payment Received Date","Enrollment Date","Enrolment Date","Enrolled On","Payment Date","Payment_Received_Date"])

def _guess_trial_sched_col(df):
    return _col_exists(df, ["Trial Scheduled Date","Trial Schedule Date","Trial Booking Date","Trial Booked Date","Trial_Scheduled_Date"])

def _guess_trial_resched_col(df):
    return _col_exists(df, ["Trial Rescheduled Date","Trial Re-scheduled Date","Trial Reschedule Date","Trial_Rescheduled_Date"])

def _guess_calib_done_col(df):
    return _col_exists(df, ["Calibration Done Date","Calibration Completed Date","First Calibration Done Date","Calibration Booking Date","Calibration Booked Date","First Calibration Scheduled Date"])

def _ensure_datetime(df, col):
    if col is None: return _pd.Series(_pd.NaT, index=df.index)
    s = df[col]
    if _pd.api.types.is_datetime64_any_dtype(s):
        return s
    return _dt_parse_dayfirst(s)

def _render_marketing_kids_detail(df):
    _st.subheader("Kids detail – 's kid' pattern (e.g., \"S kid\", \"s kid\")")
    if df is None or not isinstance(df, _pd.DataFrame) or df.empty:
        _st.info("No data available."); return

    deal_col = _guess_dealname_col(df)
    create_col = _guess_create_col(df)
    pay_col = _guess_enroll_col(df)
    trial_s_col = _guess_trial_sched_col(df)
    trial_r_col = _guess_trial_resched_col(df)
    calib_d_col = _guess_calib_done_col(df)
    source_col = _guess_source_col(df)

    if deal_col is None:
        _st.error("Could not find the Deal Name column."); return

    create_dt = _ensure_datetime(df, create_col)
    pay_dt = _ensure_datetime(df, pay_col)
    trial_s_dt = _ensure_datetime(df, trial_s_col)
    trial_r_dt = _ensure_datetime(df, trial_r_col)
    calib_d_dt = _ensure_datetime(df, calib_d_col)

    c1, c2, c3, c4 = _st.columns(4)
    with c1:
        mode = _st.selectbox("Mode", ["Entity", "Cohort"], index=0, key="mk_kids_mode")
    with c2:
        dflt_start = (create_dt.min() if create_dt.notna().any() else _pd.Timestamp("2023-01-01"))
        start = _st.date_input("Start date", value=(dflt_start.date() if _pd.notna(dflt_start) else _pd.Timestamp("2023-01-01").date()), key="mk_kids_start")
    with c3:
        dflt_end = (create_dt.max() if create_dt.notna().any() else _pd.Timestamp.today())
        end = _st.date_input("End date", value=(dflt_end.date() if _pd.notna(dflt_end) else _pd.Timestamp.today().date()), key="mk_kids_end")
    with c4:
        only_organic = _st.checkbox("Only Organic?", value=True, key="mk_kids_only_org")

    # Match: word-boundary 's' followed by whitespace then 'kid' (case-insensitive).
    # Examples: "S kid", "s   kid", "xyz S kid abc"
    name_pat = _re.compile(r"\\bs\\s+kid\\b", flags=_re.IGNORECASE)
    is_kids = df[deal_col].astype(str).str.contains(name_pat)

    if source_col is not None and only_organic:
        is_org = df[source_col].astype(str).str.lower().str.contains("organic")
    else:
        is_org = _pd.Series(True, index=df.index)
    ...
    if mode == "Entity":
        f_create = in_range(create_dt)
        f_pay = in_range(pay_dt)
        f_trial_s = in_range(trial_s_dt)
        f_trial_r = in_range(trial_r_dt)
        f_calib_d = in_range(calib_d_dt)
    else:
        f_base = in_range(create_dt)
        f_create = f_base
        f_pay = f_base
        f_trial_s = f_base
        f_trial_r = f_base
    ...
    def n_unique(mask):
        return df.loc[mask, deal_col].nunique()

    cnt_created_kids = n_unique(base_kids & f_create)
    cnt_trial_s_kids = n_unique(base_kids & f_trial_s)
    cnt_trial_r_kids = n_unique(base_kids & f_trial_r)
    cnt_calib_d_kids = n_unique(base_kids & f_calib_d)
    cnt_enroll_kids = n_unique(base_kids & f_pay)
    ...
    denom_org = n_unique(base_org & f_create) if base_org.any() else 0
    denom_all = n_unique((create_dt.notna()) & f_create)

    def pct(part, whole):
        return (part/whole*100.0) if whole else 0.0

    _st.markdown("### Funnel – Kids Deals (matching **“s kid”**)")
    m1, m2, m3, m4, m5 = _st.columns(5)
    with m1: _st.metric("Created (Kids)", cnt_created_kids, delta=f"{pct(cnt_created_kids, denom_org):.1f}% of Organic | {pct(cnt_created_kids, denom_all):.1f}% of All")
    with m2: _st.metric("Trial Scheduled (Kids)", cnt_trial_s_kids)
    with m3: _st.metric("Trial Rescheduled (Kids)", cnt_trial_r_kids)
    with m4: _st.metric("Calibration Done (Kids)", cnt_calib_d_kids)
    with m5: _st.metric("Enrollments (Kids)", cnt_enroll_kids)
    ...
    _st.dataframe(table)
    _st.download_button("Download table (CSV)", data=table.to_csv(index=False).encode("utf-8"), file_name="kids_detail_funnel.csv", mime="text/csv", key="dl_mk_kids_table")

# Register the pill non-invasively
try:
    if 'MASTER_SECTIONS' in globals():
        if "Marketing" in MASTER_SECTIONS and "Kids detail" not in MASTER_SECTIONS["Marketing"]:
            MASTER_SECTIONS["Marketing"].append("Kids detail")
    if 'RENDERERS' in globals():
        RENDERERS[("Marketing","Kids detail")] = lambda: _render_marketing_kids_detail(df)
except Exception:
    pass
# === /MARKETING: Kids detail ===
...
# === MARKETING: Kids detail render glue (multi-router support) ===
def __kids_detail_register():
    sec = "Marketing"
    pill_name = "Kids detail"
    g = globals()
    # Ensure pill listed
    try:
        if 'MASTER_SECTIONS' in g and isinstance(MASTER_SECTIONS, dict):
            if sec in MASTER_SECTIONS and pill_name not in MASTER_SECTIONS[sec]:
                MASTER_SECTIONS[sec].append(pill_name)
    except Exception:
        pass
    # Register in common registries
    try:
        if 'RENDERERS' in g and isinstance(RENDERERS, dict):
            RENDERERS[(sec, pill_name)] = (lambda: _render_marketing_kids_detail(g.get('df', g.get('df_f', None))))
    except Exception:
        pass
    try:
        if 'PILL_RENDERERS' in g and isinstance(PILL_RENDERERS, dict):
            if sec not in PILL_RENDERERS: PILL_RENDERERS[sec] = {}
            PILL_RENDERERS[sec][pill_name] = (lambda: _render_marketing_kids_detail(g.get('df', g.get('df_f', None))))
    except Exception:
        pass
    # Wrap a generic router if present
    try:
        if 'render_router' in g and callable(g['render_router']):
            __orig_render_router = g['render_router']
            def __wrapped_render_router(section, pill):
                if section == sec and pill == pill_name:
                    return _render_marketing_kids_detail(g.get('df', g.get('df_f', None)))
                return __orig_render_router(section, pill)
            g['render_router'] = __wrapped_render_router
    except Exception:
        pass

try:
    # call registration right away so the pill renders
    __kids_detail_register()
except Exception:
    pass
# === /render glue ===
