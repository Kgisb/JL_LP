...
# --- Kids detail renderer (early definition to avoid NameError) ---
try:
    _render_marketing_kids_detail
except NameError:
    def _render_marketing_kids_detail(df):
        ...
        def _dt_parse_dayfirst(series):
            ...
            if need.any():
                ...
                dt = dt.fillna(dt3)
            return dt
        def _guess(df, opts): return _col_exists(df, opts)
        deal_col = _guess(df, ["Deal Name","Deal name","Name","Deal","Title"])
        create_col = _guess(df, ["Create Date","Created Date","Deal Create Date","Date Created","Created On","Creation Date","Deal Created Date","Create_Date"])
        pay_col = _guess(df, ["Payment Received Date","Enrollment Date","Enrolment Date","Enrolled On","Payment Date","Payment_Received_Date"])
        trial_s_col = _guess(df, ["Trial Scheduled Date","Trial Schedule Date","Trial Booking Date","Trial Booked Date","Trial_Scheduled_Date"])
        trial_r_col = _guess(df, ["Trial Rescheduled Date","Trial Re-scheduled Date","Trial Reschedule Date","Trial_Rescheduled_Date"])
        calib_d_col = _guess(df, ["Calibration Done Date","Calibration Completed Date","First Calibration Done Date","Calibration Booking Date","Calibration Booked Date","First Calibration Scheduled Date"])
        source_col = _guess(df, ["JetLearn Deal Source","Deal Source","Original source","Source","Original traffic source"])
        if deal_col is None:
    ...
    # Allow user to refine by exact names that matched the pattern
    try:
        _candidates = sorted(df.loc[is_kids, deal_col].astype(str).unique().tolist())
        _selected_names = __import__('streamlit').multiselect("Names (auto-matched)",
                                                              options=_candidates,
                                                              default=_candidates,
                                                              key="mk_kids_names")
        if _selected_names:
            is_kids = is_kids & df[deal_col].astype(str).isin(_selected_names)
    except Exception:
        pass

        is_org = df[source_col].astype(str).str.lower().str.contains("organic") if (source_col and only_organic) else _pd.Series(True, index=df.index)
        start_ts = _pd.Timestamp(start); end_ts = _pd.Timestamp(end) + _pd.Timedelta(days=1) - _pd.Timedelta(seconds=1)
        in_range = lambda s: (s >= start_ts) & (s <= end_ts)
        if mode == "Entity":
            f_create, f_pay, f_trial_s, f_trial_r, f_calib_d = in_range(create_dt), in_range(pay_dt), in_range(trial_s_dt), in_range(trial_r_dt), in_range(calib_d_dt)
        else:
        ...
        st.download_button("Download table (CSV) — Kids detail", data=table.to_csv(index=False).encode("utf-8"),
                           file_name="kids_detail_funnel.csv", mime="text/csv", key="dl_mk_kids_table_v8")
# --- /Kids detail renderer (early) ---

# === CLEAN redefine: _render_marketing_kids_detail ===
def _render_marketing_kids_detail(df):
    import pandas as _pd, numpy as _np, re as _re, streamlit as st
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        mode = st.selectbox("Mode", ["Entity", "Cohort"], index=0, key="mk_kids_mode")
    with c2:
        dflt_start = (create_dt.min() if create_dt.notna().any() else _pd.Timestamp("2023-01-01"))
        start = st.date_input("Start date", value=(dflt_start.date() if _pd.notna(dflt_start) else _pd.Timestamp("2023-01-01").date()), key="mk_kids_start")
    with c3:
    ...
    with c4:
        only_organic = st.checkbox("Only Organic?", value=True, key="mk_kids_only_org")

    # --- matching logic: 's kid' variants ---
    name_pat = _re.compile(r"(?:^|\b)[’']?\s*s\s*[-_./]*\s*kid\b", flags=_re.IGNORECASE)
    is_kids = df[deal_col].astype(str).str.contains(name_pat)
    ...
    # --- source filter and date windows ---
    is_org = df[source_col].astype(str).str.lower().str.contains("organic") if (source_col and only_organic) else _pd.Series(True, index=df.index)
    ...
    st.dataframe(table, use_container_width=True)
    st.download_button("Download table (CSV) — Kids detail", data=table.to_csv(index=False).encode("utf-8"),
                       file_name="kids_detail_funnel.csv", mime="text/csv", key="dl_mk_kids_table_clean")
# === /CLEAN redefine ===
if view == "Kids detail":
...
def _render_marketing_kids_detail(df):
    ...
    st.dataframe(table, use_container_width=True)
    st.download_button("Download table (CSV) — Kids detail", data=table.to_csv(index=False).encode("utf-8"),
                       file_name="kids_detail_funnel.csv", mime="text/csv", key="dl_mk_kids_table_v7")
