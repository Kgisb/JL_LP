...
def _ctt_parse_date_ddmmyyyy(val) -> date | None:
    """Parse dates in DD-MM-YYYY / DD/MM/YYYY / DD.MM.YYYY / DD MM YYYY, also DDMMYYYY,
    and two-digit year variants. Accept Excel serials too.
    ...
    s = str(val).strip()
    # Excel serials
    if s.isdigit() and 0 < int(s) < 600000:
        try:
            base = datetime(1899, 12, 30)  # pandas/Excel origin
            return (base + pd.to_timedelta(int(s), unit='D')).date()
        except Exception:
            pass
    # Pure 8-digit DDMMYYYY
    if s.isdigit() and len(s) == 8:
        try:
            return datetime.strptime(s, '%d%m%Y').date()
        except Exception:
            pass
    # Separated formats, including 2-digit year
    for fmt in ('%d/%m/%Y','%d-%m-%Y','%d.%m.%Y','%d %m %Y','%d/%m/%y','%d-%m-%y','%d.%m.%y','%d %m %y'):
        try:
            return datetime.strptime(s, fmt).date()
        except Exception:
            continue
    # Last resort: pandas with dayfirst
    try:
...
def _ctt_build_dt(df: pd.DataFrame, date_col='Date', time_col='Time') -> pd.DataFrame:
    """Create _dt by combining parsed date & time directly (no string parsing)."""
    d_series = df[date_col].apply(_ctt_parse_date_ddmmyyyy)
    t_series = df[time_col].apply(_ctt_parse_time_only)
    def _comb(d, t):
        try:
            if d is None or t is None: return pd.NaT
            return datetime.combine(d, t)
        except Exception:
            return pd.NaT
    dt = [ _comb(d, t) for d, t in zip(d_series, t_series) ]
    out = df.assign(_dt=pd.to_datetime(dt, errors='coerce')).dropna(subset=['_dt'])
    return out
...
def _render_call_talktime_report():
    ...
    def _ctt_fmt_ddmmyyyy(df):
        for c in df.columns:
            if 'date' in c.lower() or c.strip().lower() in {'_d','_dt'}:
                try:
    ...
    if upl is None:
        st.info("No CSV uploaded. Trying to use in-app data (if it has call log columns).")
        _cand = None
        try:
            # Prefer a preloaded DataFrame if available in globals/session
            _cand = globals().get("df_f", None)
        except Exception:
            _cand = None
        if _cand is None:
            try:
                _cand = pd.read_csv(DEFAULT_DATA_PATH)
            except Exception:
                _cand = None
        if _cand is None:
            st.warning("Could not find any in-app data with call logs. Please upload activityFeedReport_*.csv.")
            return
        df_raw = _cand.copy()
    try:
    ...
    # Resolve columns (kept as-is)
    # --- Enforce day-first parsing strictly ---
    # Keep raw string dates (do NOT let pandas auto-parse)
    if date_col not in df_raw.columns:
        st.error('Call Talk-time: missing Date column.'); return
    # Create parsed date & time columns explicitly
    _date_parsed = df_raw[date_col].apply(_ctt_parse_date_ddmmyyyy)
    _time_parsed = df_raw[time_col].apply(_ctt_parse_time_only) if time_col in df_raw.columns else None
    def _comb(d, t):
        try:
            if d is None or t is None: return pd.NaT
            return datetime.combine(d, t)
            ...
            return pd.NaT
    _dt = pd.to_datetime([_comb(d, t) for d, t in zip(_date_parsed, _time_parsed)], errors='coerce')
    df = df_raw.copy()
    df['__date_df'] = _date_parsed
    df['_dt'] = _dt
    # Guard: if nothing parsed, stop early
    if df['_dt'].isna().all():
        st.error('Could not parse any dates. Ensure sheet Date is DD-MM-YYYY / DD/MM/YYYY / DDMMYYYY.'); return
    # Confirmation line to avoid DD/MM vs MM/DD confusion
    _mn = pd.to_datetime(df['_dt'].min()).strftime('%d-%m-%Y')
    _mx = pd.to_datetime(df['_dt'].max()).strftime('%d-%m-%Y')
    st.caption(f'Parsed (day‑first) date range in file: **{_mn} → {_mx}** (DD‑MM‑YYYY)')
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        d_start = st.date_input("Start date", value=min_d, min_value=min_d, max_value=max_d, key="ctt_start_d")  # (DD-MM-YYYY)
    with c2:
        d_end = st.date_input("End date", value=max_d, min_value=min_d, max_value=max_d, key="ctt_end_d")  # (DD-MM-YYYY)
    st.caption(f"Selected window: **{d_start.strftime('%d-%m-%Y')} → {d_end.strftime('%d-%m-%Y')}** (DD‑MM‑YYYY)")
    with c3:
    ...
    df_win = df_win.copy(); df_win['_d_fmt'] = df_win['_dt'].dt.strftime('%d-%m-%Y')
...
try:
    ...
    import streamlit as st
    st.error(f"Call Talk-time Report error: {e}")
