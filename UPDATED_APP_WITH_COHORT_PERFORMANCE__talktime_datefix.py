...
# === DATE FIX: Unified day-first parsing for all date-like columns (DD-MM-YYYY / DD/MM/YYYY / DDMMYYYY) ===
import pandas as _pd
import numpy as _np

def _looks_like_date_column(colname: str) -> bool:
    if not isinstance(colname, str):
        return False
    cn = colname.lower()
    # include most date/time fields used in the app, including Talk-time related sheets
    keys = [
        "date","time","slot","created","create","payment","enrol","enroll","first cal",
        "calibration","resched","done","book","activity","trigger","trial","call"
    ]
    return any(k in cn for k in keys)

def _parse_compact_ddmmyyyy(val):
    # Handle 8-digit compact form like "01102025" -> 01-10-2025 (1 Oct 2025)
    s = str(val).strip()
    if re.fullmatch(r"\\d{8}", s):
        try:
            return _pd.to_datetime(s, format="%d%m%Y", errors="coerce")
        except Exception:
            return _pd.NaT
    return _pd.NaT

def _safe_to_datetime(series: _pd.Series):
    # Try day-first, then generic, then compact DDMMYYYY, then extract leading 10 chars if mixed strings
    s = series.copy()
    # First quick path: already datetime-like
    if _pd.api.types.is_datetime64_any_dtype(s) or _pd.api.types.is_datetime64tz_dtype(s):
        return _pd.to_datetime(s, errors="coerce")
    # Try day-first
    parsed = _pd.to_datetime(s, errors="coerce", infer_datetime_format=True, dayfirst=True)
    if parsed.notna().sum() >= max(3, int(0.1 * len(s))):
        return parsed
    # Try generic parser
    parsed2 = _pd.to_datetime(s, errors="coerce", infer_datetime_format=True)
    if parsed2.notna().sum() >= max(3, int(0.1 * len(s))):
        return parsed2
    # Try compact ddmmyyyy
    compact = s.map(_parse_compact_ddmmyyyy)
    if compact.notna().sum() >= max(3, int(0.1 * len(s))):
        return compact
    # Try extracting first 10 chars (e.g., '01-10-2025 18:30' or '01/10/2025 18:30')
    lead10 = s.astype(str).str.slice(0, 10).str.replace(".", "-", regex=False).str.replace("/", "-", regex=False)
    parsed3 = _pd.to_datetime(lead10, errors="coerce", dayfirst=True)
    if parsed3.notna().sum() >= max(3, int(0.1 * len(s))):
        return parsed3
    return parsed  # fallback (may be mostly NaT)

def _unify_dates_dayfirst(df: _pd.DataFrame) -> _pd.DataFrame:
    df = df.copy()
    for c in df.columns:
        if _looks_like_date_column(c):
            try:
                coerced = _safe_to_datetime(df[c])
                # Only replace if we were able to parse a meaningful portion
                if coerced.notna().sum() >= max(3, int(0.1 * len(coerced))):
                    df[c] = coerced
            except Exception:
                # Best-effort; ignore non-date columns
                pass
    return df
# === /DATE FIX ===
...
# DATE FIX: normalize all date-like columns to day-first
df = _unify_dates_dayfirst(df)
# Column mapping
