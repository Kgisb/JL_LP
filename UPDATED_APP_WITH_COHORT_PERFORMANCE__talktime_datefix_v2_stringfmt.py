...
# === DATE FIX V2: Force all date-like columns to 'YYYY/MM/DD' STRING format after day-first parsing ===
...
def _looks_like_date_column(colname: str) -> bool:
    ...
    cn = colname.lower()
    keys = [
        "date","time","slot","created","create","payment","enrol","enroll",
        "first cal","calibration","resched","done","book","activity","trigger",
        "trial","call"
    ]
    return any(k in cn for k in keys)

def _parse_dayfirst(series: _pd.Series):
    # parse robustly as day-first
    s = series.astype(str).str.strip()
    # common replacements
    s = s.str.replace(".", "-", regex=False).str.replace("/", "-", regex=False)
    # try direct parse
    dt = _pd.to_datetime(s, errors="coerce", dayfirst=True, infer_datetime_format=True)
    # try compact DDMMYYYY as fallback
    need = dt.isna()
    if need.any():
        ...
        dt = dt.fillna(dt2)
    # try taking first 10 chars as DD-MM-YYYY
    need = dt.isna()
    if need.any():
...
def _format_dates_yyyy_slash_mm_slash_dd(df: _pd.DataFrame) -> _pd.DataFrame:
    df = df.copy()
    for c in df.columns:
        if _looks_like_date_column(c):
            try:
                dt = _parse_dayfirst(df[c])
                # only convert if meaningful
                if dt.notna().sum() >= max(3, int(0.1 * len(dt))):
                    # keep a hidden datetime version for any .dt operations if app uses it
                    hidden_col = f"__dt__{c}"
                    if hidden_col not in df.columns:
                        df[hidden_col] = dt
                    # overwrite visible col as string 'YYYY/MM/DD'
...
# DATE FIX V2: reformat date-like columns to 'YYYY/MM/DD' strings
df = _format_dates_yyyy_slash_mm_slash_dd(df)
# Column mapping
