...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity", "Activity Tracker", "Deal stage", "Original source", "Referral / No-Referral", "Lead mix", "Referral performance", "Activity concentration"],
...
def _render_performance_referral_performance(
    ...
    with col_f2:
        only_referrals = st.checkbox("Show only 'Referrals'", value=False, key="rp_only_referrals")

    if hide_unknown:
        df = df[df["__ref"] != "Unknown"]
    if only_referrals:
        df = df[df["__ref"] == "Referrals"]
    ...
    if _bcol and _bcol in df.columns:
        ...
        if metric == "Deals":
            filt = mask_by(create_col) if view_mode == "Cohort" else cohort_subset.index.isin(base_for_break.index)
            dfm = base_for_break.loc[filt] if isinstance(filt, pd.Series) else base_for_break.copy()
        else:
            if view_mode == "Cohort":
                enrol_mask = mask_by(pay_col)
                dfm = base_for_break.loc[enrol_mask.reindex(base_for_break.index, fill_value=False)].copy()
            else:
