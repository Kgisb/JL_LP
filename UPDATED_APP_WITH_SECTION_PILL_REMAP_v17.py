...
with st.sidebar:
    ...
    MASTER_SECTIONS = {'Performance': ['Cash-in',
                 'Quick View',
                 'MIS',
                 'Dashboard',
                 'Leaderboard',
 ...
 'Insights & Forecast': ['Predictibility',
                         ...
                         'Heatmap',
                         'Master Graph'],
 'Funnel & Movement': ['Trend & Analysis',
                       'Funnel',
                       'Lead Movement',
                       'Carry Forward',
                       'Deal Velocity',
                       ...
                       'Stuck deals',
                       'Slow Working Deals'],
 'Marketing': ['Referral / No-Referral',
               ...
               'Deal stage',
               'Original source',
               'HubSpot Deal Score tracker',
               'Marketing Lead Performance & Requirement']}
...
# === Lifted renderer: Activity concentration ===
...
# === Lifted renderer: Slow Working Deals ===
...
# === Extracted renderer: Stuck deals ===
def _render_funnel_stuck_deals(df_f, create_col, pay_col, country_col, source_col, counsellor_col):
...
# === Lifted renderer: Referral / No-Referral ===
def _render_performance_referral_no_referral(
    ...
    # Force date columns to pure date to avoid dtype mismatch
    try:
        if create_col and create_col in df_f.columns:
            df_f[create_col] = pd.to_datetime(df_f[create_col], errors="coerce").dt.date
        if pay_col and pay_col in df_f.columns:
            df_f[pay_col] = pd.to_datetime(df_f[pay_col], errors="coerce").dt.date
    except Exception:
        pass
    # --- Normalize: ensure comparable datetime types ---
    import pandas as _pd_fix
    if create_col and create_col in df_f.columns:
        try: df_f[create_col] = _pd_fix.to_datetime(df_f[create_col], errors="coerce")
        except Exception: pass
    if pay_col and pay_col in df_f.columns:
        try: df_f[pay_col] = _pd_fix.to_datetime(df_f[pay_col], errors="coerce")
        except Exception: pass
    for _v in ["today","yday","last_m_start","last_m_end","this_m_start","this_m_end","this_m_end_mtd",
               "start_dt","end_dt","date_start","date_end","a_start","a_end","b_start","b_end","cur_start","cur_end"]:
        if _v in locals():
            try: locals()[_v] = _pd_fix.to_datetime(locals()[_v])
            except Exception: pass
    ...
    with cA:
        ...
        if presetA == "Today":
            ...
            a_start = pd.to_datetime(a_start)  # normalize
            with c2: a_end   = st.date_input("End (A)",   value=today, key="refnr_end_a")
            a_end   = pd.to_datetime(a_end)  # normalize
            if a_end < pd.to_datetime(a_start): a_start, a_end = a_end, a_start
    ...
    with cB:
        ...
        if presetB == "Today":
            ...
            b_start = pd.to_datetime(b_start)  # normalize
            with c2: b_end   = st.date_input("End (B)",   value=today, key="refnr_end_b")
            b_end   = pd.to_datetime(b_end)  # normalize
            if b_end < pd.to_datetime(b_start): b_start, b_end = b_end, b_start
...
# =========================
    ...
    _render_funnel_stuck_deals(df_f, create_col, pay_col, country_col, source_col, counsellor_col)

elif view == "Slow Working Deals":
    try:
        _render_performance_slow_working_deals(
            df_f=df_f,
            create_col=create_col,
            ...
            times_contacted_col=None,
            sales_activity_col=None,
        )
    except Exception as e:
        import traceback
        st.error("Slow Working Deals error: " + str(e))
        st.code(traceback.format_exc())
    import pandas as pd, numpy as np, altair as alt
    from datetime import date, timedelta
    from calendar import monthrange
    # (moved Stuck deals content into its own renderer)
...
# Referrals Tab (full)
# =========================
elif view == "Referral / No-Referral":
    try:
        _render_performance_referral_no_referral(df_f, create_col, pay_col, source_col, country_col, counsellor_col)
    except Exception as e:
        import traceback
        st.error("Referral / No-Referral failed: " + str(e))
        st.code(traceback.format_exc())
...
elif view == "Activity concentration":
    try:
        _render_performance_activity_concentration(
            df_f=df_f,
            create_col=create_col,
        ...
        import traceback
        st.error("Activity concentration failed: " + str(e))
